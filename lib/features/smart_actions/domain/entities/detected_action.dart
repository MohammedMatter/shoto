...
/// double-check is worse than no action at all, which is why `phone` used to
/// be on this list and is not any more: see [ActionExtractor].
///
/// The first four are *intents* rather than entities, and that distinction is
/// the point. An IBAN is a thing on the screen; an invitation is something the
/// user is about to have to do. Finding the thing is easy and finding the
/// intent is what saves them the four taps — so the intents lead the enum,
/// because **declaration order is display order**: the list is sorted by
/// `kind.index`, and the row somebody actually came for has to be the row
/// they see first.
...
class DetectedAction {
  ...
  /// Cleaned-up form used to actually perform the action — an IBAN stripped of
  /// spaces, a link with its scheme filled in.
  ///
