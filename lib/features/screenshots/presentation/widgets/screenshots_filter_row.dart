...
/// The library's filter strip: **one axis, one row.**
///
/// Status ("have I dealt with this") — All / Unsorted / Favorites, mutually
/// exclusive.
///
/// **There used to be a second row underneath**, carrying the content traits
/// (Sensitive / Links / Contacts / Codes / Dates) as a separate scrolling
/// axis. Both rows overflowed the screen edge, and together with the title,
/// the count and four header buttons they spent roughly a third of a phone
/// screen before the first thumbnail — in an app whose whole job is showing
/// thumbnails. Two independent scrolling chip axes is also a power-user
/// control, and it was the second thing a first-run user met.
///
/// The traits now live in [showLibraryViewSheet], beside the sort order,
/// which is the same question asked twice: neither changes what is in the
/// library, only what you are currently looking at.
///
/// This row stays because it is the primary axis and because it cannot be put
/// behind anything: **Unsorted is the same number Home puts at 58px**, so
/// arriving from Home's hero has to land on a pill already lit and already
/// agreeing with the figure that was tapped.
